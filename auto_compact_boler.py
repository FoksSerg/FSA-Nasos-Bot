...
# -*- coding: utf-8 -*-
"""
Автоматический создатель компактных версий RouterOS скриптов для модулей Boler
...
def find_project_root():
    ...
    for search_dir in search_paths:
        ...
        if (search_dir / "RazrabNasos").exists() or (search_dir / "RazrabBoler").exists() or (search_dir / "CodeBoler").exists() or (search_dir / "MikrotikUploader").exists():
            return search_dir
...
def ensure_directories(project_root):
    ...
    razrab_dir = project_root / "RazrabBoler"
    code_dir = project_root / "CodeBoler"
    
    if not razrab_dir.exists():
        print(f"📁 Создаю папку исходников Boler: {razrab_dir}")
        razrab_dir.mkdir(exist_ok=True)
    
    if not code_dir.exists():
        print(f"📁 Создаю папку компактных версий Boler: {code_dir}")
...
def auto_process_project():
    ...
    razrab_boler_dir, code_boler_dir = ensure_directories(project_root)
    
    # Ищем .rsc файлы в папке RazrabBoler
    rsc_files = list(razrab_boler_dir.glob("*.rsc"))
    ...
    for file_path in rsc_files:
        ...
        if ("-Compact" not in filename and 
            ...
            filename.startswith("Boler-") and
            not filename.endswith(".template")):  # Исключаем шаблоны
            project_modules.append(file_path)
    
    if not project_modules:
        return [(False, f"Модули проекта Boler-*.rsc не найдены в папке {razrab_boler_dir}", 0, 0, 0)]
    
    results = []
    for module_path in project_modules:
        output_path = code_boler_dir / module_path.name
...
def run_auto_compact():
    ...
    print("=== Автоматический создатель компактных RouterOS скриптов для модулей Boler ===")
    print("Автор: Фокин Сергей Александрович")
    print("Дата: 28 июня 2025")
    print()
    ...
    print()
    print("📁 Исходники: RazrabBoler/ → Компактные версии: CodeBoler/")
    print("-" * 80)
