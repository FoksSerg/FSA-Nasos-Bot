# BOLER SSH TEST - Тестирование SSH-сервера на Boler
# Этот скрипт тестирует SSH-сервер на роутере Boler и проверяет
# готовность к приему подключений от роутера Nasos.
#
# Функции:
# 1. Проверка состояния SSH-сервера
# 2. Тест доступности глобальных переменных
# 3. Проверка импортированных SSH-ключей
# 4. Диагностика настроек безопасности
#
...
if ($userExists > 0) do={
    ...
    # Проверяем SSH-ключи пользователя
...
# === ТЕСТ 4: ПРОВЕРКА ИМПОРТИРОВАННЫХ SSH-КЛЮЧЕЙ ===
...
# === ТЕСТ 5: ПРОВЕРКА ГЛОБАЛЬНЫХ ПЕРЕМЕННЫХ ===
...
# === ТЕСТ 6: ПРОВЕРКА СКРИПТОВ BOLER ===
...
# === ТЕСТ 7: ПРОВЕРКА СЕТЕВОЙ ДОСТУПНОСТИ ===
...
# === ТЕСТ 8: ЛОКАЛЬНЫЙ ТЕСТ SSH-СОЕДИНЕНИЯ ===
...
# Проверяем настройки файрвола для SSH
...
# Проверяем список разрешенных адресов для SSH
